...
def plot_transport_column(
    ...
    mQ = model.result["mQ"][
        :, i, list(model._fluxorder).index(flux), list(model._solorder).index(sol)
    ]
    ...
    if do_init:
        ...
        artists_dict[f"TCpatch C_old"] = patches.Rectangle(
        ...
        if total_storage is not None:
            artists_dict[f"TCpatch below total"] = patches.Rectangle(
                (0, 0), 0, 0, linewidth=0, edgecolor="w", facecolor="w", visible=False
            )  # , clip_on=False)
    ...
    if total_storage is not None:
        artists_dict[f"TCpatch below total"].set_bounds(
            (0, total_storage[i], 1, ST_max - total_storage[i])
        )
    ...
    for n in range(nST):
        artists_dict[f"TQpatch {n}"].set_bounds(
            (0, ST_reg[n], omega_reg[n], sT_reg[n] * dt - spacer)
        )
    ...
    if do_init:
        ...
        (artists_dict[f"omegaline v start"],) = ax2.plot(
            [0, 0], [0, 0], omega_linestylestr
        )  # , clip_on=False)
        (artists_dict[f"omegaline h start"],) = ax2.plot(
            [0, 0], [0, 0], omega_linestylestr
        )  # , clip_on=False)
        for ip in range(len(omega_mod)):
            (artists_dict[f"omegaline v {ip}"],) = ax2.plot(
                [0, 0], [0, 0], omega_linestylestr
            )  # , clip_on=False)
            if ip > 0:
                (artists_dict[f"omegaline h {ip}"],) = ax2.plot(
                    [0, 0], [0, 0], omega_linestylestr
                )  # , clip_on=False)
        (artists_dict[f"omegaline h end"],) = ax2.plot(
            [0, 0], [0, 0], omega_linestylestr
        )  # , clip_on=False)
        (artists_dict[f"omegaline v end"],) = ax2.plot(
            [0, 0], [0, 0], omega_linestylestr
        )  # , clip_on=False)

    artists_dict[f"omegaline v start"].set_data([0, 0], [0, ST_mod[0]])
    artists_dict[f"omegaline h start"].set_data(
        [0, omega_mod[0]], [ST_mod[0], ST_mod[0]]
    )
    if piecewise_omega:
        for ip in range(len(omega_mod)):
            artists_dict[f"omegaline v {ip}"].set_data(
                [omega_mod[ip], omega_mod[ip]], [ST_mod[ip], ST_mod[ip + 1]]
            )
            if ip > 0:
                artists_dict[f"omegaline h {ip}"].set_data(
                    [omega_mod[ip - 1], omega_mod[ip]], [ST_mod[ip], ST_mod[ip]]
                )
        artists_dict[f"omegaline h end"].set_data(
            [omega_mod[-1], 0], [ST_mod[-1], ST_mod[-1]]
        )
    else:
        for ip in range(1, len(omega_mod)):
            artists_dict[f"omegaline v {ip}"].set_data(
                [omega_mod[ip - 1], omega_mod[ip]], [ST_mod[ip], ST_mod[ip + 1]]
            )
            # if ip > 0:
            # artists_dict[f'omegaline h {ip}'].set_data([omega_mod[ip - 1], omega_mod[ip]], [ST_mod[ip], ST_mod[ip]])
    if total_storage is not None:
        artists_dict[f"omegaline v end"].set_data(
            [0, 0], [ST_mod[-1], total_storage[i]]
        )
    else:
...
def plot_influx(
    model, ax=None, sharex=None, i=0, artists_dict=OrderedDict(), do_init=True
):
    ...
    if do_init:
        ...
        (artists_dict[f"plot_influx timeline"],) = ax.plot(
            2 * [model.data_df.index[0]], [0, 0], "k", lw=0.5
        )
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
        ax.set_title(model.options["influx"])
    if i is not None:
        artists_dict[f"plot_influx timeline"].set_data(
            2 * [model.data_df.index[i]], ax.get_ylim()
        )


def plot_outflux(
    model, flux, ax=None, sharex=None, i=0, artists_dict=OrderedDict(), do_init=True
):
    ...
    if do_init:
        ...
        ax.set_ylim(bottom=0)
        (artists_dict[f"plot_outflux timeline"],) = ax.plot(
            2 * [model.data_df.index[0]], [0, 0], "k", lw=0.5
        )
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
        ax.set_title(flux)
    if i is not None:
        artists_dict[f"plot_outflux timeline"].set_data(
            2 * [model.data_df.index[i]], ax.get_ylim()
        )


def plot_influx_conc(
    model, sol, ax=None, sharex=None, i=0, artists_dict=OrderedDict(), do_init=True
):
    ...
    if do_init:
        ...
        (artists_dict[f"plot_influx_conc timeline"],) = ax.plot(
            2 * [model.data_df.index[0]], [0, 0], "k", lw=0.5
        )
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
        ax.set_title(sol)
    if i is not None:
        artists_dict[f"plot_influx_conc timeline"].set_data(
            2 * [model.data_df.index[i]], ax.get_ylim()
        )


def plot_outflux_conc(
    model,
    flux,
    sol,
    ax=None,
    ...
    if do_init:
        ...
        (artists_dict[f"plot_outflux_conc timeline"],) = ax.plot(
            2 * [model.data_df.index[0]], [0, 0], "k", lw=0.5
        )
        ax.spines["right"].set_visible(False)
        ax.spines["top"].set_visible(False)
    ...
    if i is not None:
        artists_dict[f"plot_outflux_conc timeline"].set_data(
            2 * [model.data_df.index[i]], ax.get_ylim()
        )


def plot_SAS_cumulative(
    model, flux, ax=None, sharex=None, i=0, artists_dict=OrderedDict(), do_init=True
):
    model.sas_specs[flux].make_spec_ts()
    ST_mod = np.r_[0.0, model.sas_specs[flux].ST[i, :]]
...
def plot_transport_column_with_timeseries(
    model, flux, sol, i=0, fig=None, artists_dict=OrderedDict(), **kwargs
):
    if fig is None:
    ...
    plot_transport_column(
        model, flux, sol, i=i, ax=axTC, artists_dict=artists_dict, **kwargs
    )
    ...
    plot_outflux_conc(
        model, flux, sol, ax=axCQ, sharex=axJ, i=i, artists_dict=artists_dict
    )
...
def make_transport_column_animation(model, flux, sol, fig=None, frames=None, **kwargs):
    ...
    def init():
        ...
        plot_transport_column_with_timeseries(
            model, flux, sol, i=i, fig=fig, artists_dict=artists, **kwargs
        )
